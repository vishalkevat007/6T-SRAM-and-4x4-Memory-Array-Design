...
grid on;
title('Hold Static Noise Margin (HSNM)');
...
x2_second = second_myx + second_max_diagonal * cosd(225);
y2_second = second_myy + second_max_diagonal * sind(225);
...
hold_SNM = min(side_length_max, side_length_second_max);

disp(['Hold static noise margin: ', num2str(hold_SNM)]);

% Save the figure in high resolution
fig = gcf; % Get the current figure handle

% Set the resolution to 300 DPI for high quality
exportgraphics(fig, 'hold_SNM.png', 'Resolution', 300); % Save as PNG
% exportgraphics(fig, 'hold_SNM.pdf', 'Resolution', 300); % Save as PDF
